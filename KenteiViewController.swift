...
class KenteiViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        seikaiLabel.textAlignment = .Center     //(NSTextAlignment.Center)
        //正解表示ラベルのフォントサイズを15ポイントに設定
        ...
        //バックボタンにアクションを設定
        backBtn.addTarget(self, action: #selector(KenteiViewController.backBtnTapped(_:)), forControlEvents: UIControlEvents.TouchUpInside)
        //バックボタンに解説バックグラウンド画像に配置
    ...
    private func kaisetsu(){
        ...
        //解説バックグラウンド画像を表示させるあアニメーション
        UIView.animateWithDuration(0.5, animations: {() -> Void in
            self.kaisetsuBGImageView.frame = CGRect(x: self.kaisetsuBGX, y: Double(answerBtnY), width: 320, height: 210)})
        //選択肢のボタンの使用停止
    ...
    func backBtnTapped(sender: UIButton){
        ...
        UIView.animateWithDuration(0.5, animations: {() -> Void in self.kaisetsuBGImageView.frame = CGRect(x: self.kaisetsuBGX, y: screenHight, width: 320, height: 210)})
        //選択肢ボタンの使用を再開
